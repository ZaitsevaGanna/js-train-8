...
function checkArray(obj) {
  if (obj === null || obj === undefined) {
    return console.log(`Вхідний об'єкт є null або undefined`);
...
console.log(createArray()); // Виведе [1, 2, 3, 4, 5]
...
function findElementIndexes(arr, element) {
  ...
  if (Array.isArray(arr)) {
    let firstElem = arr.indexOf(element);
    // console.log(firstElem);
    let lastElem = arr.lastIndexOf(element);
    // console.log(lastElem);
    if (firstElem === lastElem) {
      return (resZero = Array.of(firstElem));
    }
    return (resZero = Array.of(firstElem, lastElem));
  }
  return resZero;
...
function copyAndSwapElements(arr, target, start, end) {
  if (start < arr.length && end < arr.length && target < arr.length) {
...
function customAt(arr, index) {
  if (Array.isArray(arr) && index < arr.length) {
    ...
    } else if (typeRes === "number") {
      console.log(`Елемент є числом`);
    } else if (typeRes === "object") {
...
function customIncludes(arr, element) {
  if (Array.isArray(arr) && arr.includes(element)) {
    const res = arr.filter((value) => value === element);
    console.log(res, res.length);
    return arr.filter((value) => value === element) ? true : false;
  }
